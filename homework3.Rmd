...
output:
  html_document:
    df_print: paged
keywords: Monte Carlo, R Markdown, box plots
fontsize: 11pt
papersize: letter
...
abstract: 
    This is a report to complete approximation of the distribution fuction of $N(0,1)$ by **the Monte Carlo methods**. From the report,we can conclude that as n gets bigger and bigger,approximation of the distribution function is getting closer and closer to the truth value.
---
# Introduction 
This is a report to complete approximation of the distribution fuction of N(0,1) by the Monte Carlo methods.  

In order to demonstrate the process of approximation,I made a table with approximate values and true values drawn some box plots of the 100 approximation errors at each t for each n.  

Finally,we can draw an conclude that as n gets bigger and bigger,approximation of the distribution function is getting closer and closer to the truth value.  

# Math Equations
Monte Carlo method. They are often used in physical and mathematical problems and are most useful when it is difficult or impossible to use other approaches. Monte Carlo methods are mainly used in three problem classes: optimization, numerical integration, and generating draws from a probability distribution .  

Here I will approximate the distribution
function of  $N(0,1)$ , 
\begin{align}
...
First,I experiment with the approximation at $n \in \lbrace10^2,10^3,10^4\rbrace$ 
at $t \in \lbrace0.0,0.67,0.84,1.28,1.65,2.32,2.58,3.09,3.72\rbrace$
to form a table.From the table we can see,n from 100 to 10000,the gap between the approximate value and the truth value is getting smaller.  
```{r norm, echo = T }  
...
library(magrittr)
library(kableExtra)
kable(p, booktabs=TRUE, caption='Comparison between approximate and true value') %>%
  kable_styling(bootstrap_options = "striped",full_width = F) %>% 
 column_spec(1,bold=T)
```
Second,I repeat the experiment 100 times.Draw box plots of the 100
...
When n is a large number, the approximate value is approximately equal to the truth value.So,by using the Monte Carlo methods,we can generatie draws from a probability distribution.
